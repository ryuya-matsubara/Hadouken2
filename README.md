# Hadouken Battle ⚡

プレイヤー1 vs プレイヤー2 のターン制対戦Webアプリ。1台の端末で交互に操作する「パス＆プレイ」方式です。**相手のHPを0にした方が勝ち**ます。

## あそび方

1. `index.html` をブラウザで開く。
2. 「ゲームスタート」を押す。
3. プレイヤー1がキャラクターを選び、「キャラクターを決定」を押す。
4. 続いてプレイヤー2がキャラクターを選び、「キャラクターを決定」を押す。
5. プレイヤー1 → プレイヤー2 の順にアクションを選択する。
6. 「⚔️Battle⚔️」ボタンでバトルシーンが再生される。
7. どちらかのHPが0になれば勝者を表示、決着がつかなければ次のターンへ。

## ルール

- HP制。**HPが0になったら負け**（🟢 が3つからスタート）。
- 1ターンに1アクションを実行する。
- 先にプレイヤー1、次にプレイヤー2がアクションを選ぶ。
- アクション選択時以外は、自分と相手の選択が見えないようになっている。
- 両者のHPが同時に0になった場合は引き分け。

## キャラクター

キャラクターごとに固有の必殺技を持ちます。

| キャラ | 必殺技 | 消費エネルギー | 効果 |
|---|---|---:|---|
| Hadou | HADOUKEN | 3 | ガード不能。相手に1ダメージ |
| Blaze | Mega Blast | 2 | 相手に2ダメージ。ただしガード可能 |
| Phantom | VOID | 1 | 相手のエネルギーを1減らす。相手が攻撃してきた場合、HPは減る |
| Angel | Heal | 2 | 自分のHPを1回復する |

## アクション

チャージ・ガード・ブラストは全キャラクター共通。必殺技だけがキャラクター固有です。

| アクション | 消費エネルギー | 効果 |
|---|---|---|
| 🔵 チャージ | +1 | エネルギーを1溜める（MAX 3） |
| 💥 ブラスト | -1 | 相手がチャージ中なら1ダメージ。ガードで防がれる |
| 🛡️ ガード | 0 | ブラスト（およびガード可能な必殺技）を防ぐ |
| ⭐ 必殺技 | キャラ依存 | キャラクター固有の効果を発動 |

- エネルギーは最大 **3** 個（🟠 で表示）。
- 消費エネルギーが足りないアクションは選択できません。

## 画像について

- キャラクターのアイコン・アクションシーンの画像はキャラクターごとに切り替わります。
- **プレイヤー2** は各画像を CSS（`transform: scaleX(-1)`）で左右反転して表示します（反転済み画像は使いません）。
- アクション選択画面では、チャージ・ガード・ブラストは共通アイコン、右下の必殺技だけキャラ固有のアイコンを表示します。

## 技術

- 依存ライブラリなしの単一 HTML ファイル（HTML + CSS + Vanilla JS）。
- ビルド不要。`index.html` を開くだけで動作します。
